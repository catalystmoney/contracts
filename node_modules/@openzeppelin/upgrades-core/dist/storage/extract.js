...
function extractStorageLayout(contractDef, decodeSrc, deref, storageLayout) {
    ...
    if (storageLayout !== undefined) {
        layout.types = (0, map_values_1.mapValues)(storageLayout.types, m => {
            ...
            return {
                label: m.label,
                members: (_a = m.members) === null || _a === void 0 ? void 0 : _a.map(m => typeof m === 'string' ? m : (0, pick_1.pick)(m, ['label', 'type', 'offset', 'slot'])),
...
function loadLayoutType(varDecl, layout, deref) {
    ...
    for (const typeName of typeNames.values()) {
        ...
        (_a = (_c = layout.types)[type]) !== null && _a !== void 0 ? _a : (_c[type] = { label });
        if ('referencedDeclaration' in typeName && !/^t_contract\b/.test(type)) {
            ...
            (_b = (_d = layout.types[type]).members) !== null && _b !== void 0 ? _b : (_d.members = getTypeMembers(typeDef));
...
function getRetypedRenamed(varDecl) {
    ...
    if ('documentation' in varDecl) {
        const docs = typeof varDecl.documentation === 'string' ? varDecl.documentation : (_b = (_a = varDecl.documentation) === null || _a === void 0 ? void 0 : _a.text) !== null && _b !== void 0 ? _b : '';
        for (const { groups } of (0, execall_1.execall)(/^\s*(?:@(?<title>\w+)(?::(?<tag>[a-z][a-z-]*))? )?(?<args>(?:(?!^\s@\w+)[^])*)/m, docs)) {
            if ((groups === null || groups === void 0 ? void 0 : groups.title) === 'custom') {
